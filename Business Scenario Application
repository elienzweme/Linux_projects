...
# If present, this file should define notify() which accepts level and message
if [[ -f /usr/local/lib/heritage_notify.sh ]]; then
  # shellcheck disable=SC1091
  source /usr/local/lib/heritage_notify.sh
else
  notify() { :; }
fi
...
log() {
  ...
  echo "[$ts] [$level] $msg" | tee -a "$LOG_FILE" | logger -t heritage_automation -p "user.${level,,}" || true; notify "$level" "$msg"
}
